...
This project uses **visual regression testing** to verify that UI components render correctly. Tests run headlessly using Qt's offscreen platform, capture widget screenshots, and compare them against stored reference images.
...
testpaths = ["tests"]
pythonpath = ["tests", "client"]
```

Both `tests/` and `client/` are on the Python path, so test modules and `ayon_ui_qt` are importable without installation.
...
The runner scans `tests/components/` for any module whose name starts with `test_`, imports it, and collects all `WidgetTest` subclasses. For each class it generates two groups of pytest items:
...
Reference images are saved in `tests/test_visual/` with the filename pattern:
...
1. Create `tests/components/test_<component>.py`.
2. Subclass `WidgetTest` and implement `build()`. Optionally implement `steps()`.

```python
# tests/components/test_my_widget.py
from qtpy.QtWidgets import QWidget
from widget_test import WidgetTest
from ayon_ui_qt.components.my_widget import AYMyWidget
...
1. Generate reference images for the new test:

```bash
pytest tests/test_visual.py -k MyWidgetTest --force-regen
```

1. Commit the generated PNG files from `tests/test_visual/`.
