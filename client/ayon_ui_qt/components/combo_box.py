...
A dropdown arrow is always drawn using the Material Symbol ``arrow_drop_down``
icon so the widget is visually recognisable as a dropdown in all use cases.
...
- ``ShortTextRole`` - an abbreviated label shown when the combo-box is in
  :attr:`~ayon_ui_qt.data_models.MenuSize.Short` mode.
...
- ``"text"``       - Full display label shown in
  :attr:`~ayon_ui_qt.data_models.MenuSize.Full` mode.
